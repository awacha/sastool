...
from misc import energycalibration, normalize_listargument
    ...
    def readpilatus300kdata(filename,dirs,quiet=False): # should work for other detectors as well
        for d in dirs:
            try:
                ...
                datastr=fid.read();
                fid.close();
                data=np.fromstring(datastr[4096:],'uint32').reshape((619,487)).astype('double')
                return data;                
            except IOError:
                pass
        if not quiet:
    ...
    for i in files:
        ...
        if h['Detector']=='Gabriel':
            print '%d\t%d\t%.1f\t%d\t%.2f\t%.4f\t%.1f\t%.f\t%s\t%s' % (
                ...
                h['PosSample'], h['Transm'], 100*np.sum(d)/h['Anode'],
