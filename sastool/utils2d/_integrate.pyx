...
cdef autoqscale(double energy, double distance, double xres, double yres,
    ...
    for ix from 0<=ix<M:
        for iy from 0<=iy<N:
            ...
            if (q1<qmin1):
                qmin1=q1
    return np.linspace(qmin1,qmax1,sqrt(M*M+N*N)/2)
...
cdef int radint_testarrays(np.ndarray[np.double_t,ndim=2] data not None,
    ...
    *M=data.shape[0];
    *N=data.shape[1];
    if dataerr.shape[0]!=(*M) or dataerr.shape[1]!=(*N) or mask.shape[0]!=(*M)
        or mask.shape[1]!=(*N):
        return 0
    return 1
...
def radint(np.ndarray[np.double_t,ndim=2] data not None,
    ...
    Inputs:
        ...
        dataerr: the error (standard deviation) matrix (of the same size as
            'data'). Should not be zero, as this is used for weighting!
        ...
        q: the q points at which the integration is requested. It should be
            defined in 1/Angstroems. Can be None. In that case, the q-range will
            be chosen automagically by taking the mask and the geometry into
            account.
        shutup: if True (default), work quietly (do not print messages).
        returnavgq: if True, returns the average q value for each bin, ie. the
            ...
            which fell into each q-bin. False by default.
        ...
        fuzzy_FWHM: FWHM for the Gauss weighing function for fuzzy integration
            ...
            from the desired q value of the bin. 0 or negative (default) means
            a standard radial averaging.
        ...
        sliceorsector: True if slice, False (default) if sector integration is
            preferred. In the former case, dphi is interpreted as the width of
            the slice in pixel units.
            ...
            for l from 0<=l<K:
                if fuzzy_FWHM<=0: # traditional integration
                    if (q1>qmax[l]): # if the q-value corresponding to the center of the current pixel
                                     # is greater than the upper limit of the l-eth bin, then skip to @IndentOk
                                     # the next bin.
                        continue
...
def radintpix(np.ndarray[np.double_t,ndim=2] data not None,
              orig,
              np.ndarray[np.uint8_t,ndim=2] mask not None,
...
def azimintq(np.ndarray[np.double_t, ndim=2] data not None,
