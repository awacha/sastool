...
class Loader(classes2.Loader):
    def __init__(self, basedir: str, recursive: bool = True, processed: bool = True, exposureclass: str = 'crd'):
        if processed:
            super().__init__([os.path.join(basedir, subdir) for subdir in ['eval2d', 'mask', 'eval1d']], recursive,
                             processed)
        else:
            super().__init__([os.path.join(basedir, subdir) for subdir in
                              ['param_override', 'param', 'images', 'mask']],
                             recursive, processed)
        self._exposureclass = exposureclass

    def loadheader(self, fsn: int) -> Header:
        return Header.new_from_file(self.find_file(self._exposureclass + '_%05d.pickle' % fsn))
